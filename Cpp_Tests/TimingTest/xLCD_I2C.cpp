...
xLCD_I2C::xLCD_I2C(byte address, byte columns, byte rows, i2c_inst * I2C, uint SDA, uint SCL) noexcept
...
inline void xLCD_I2C::I2C_Write_Byte(byte val) const noexcept
{
    static byte data;
...
void xLCD_I2C::Pulse_Enable(byte val) const noexcept
{
...
inline void xLCD_I2C::Send_Nibble(byte val) const noexcept
{
    I2C_Write_Byte(val);
    Pulse_Enable(val);
}

inline void xLCD_I2C::Send_Byte(byte val, byte mode) const noexcept
{
...
inline void xLCD_I2C::Send_Command(byte val) const noexcept
{
    Send_Byte(val, COMMAND);
}

inline void xLCD_I2C::Send_Char(byte val) const noexcept
{
    Send_Byte(val, CHAR);
}

void xLCD_I2C::Send_Register_Select(byte val) const noexcept
{
    Send_Byte(val, REGISTER_SELECT);
}

inline void xLCD_I2C::Init() noexcept
{
...
void xLCD_I2C::DisplayOn() noexcept
{
    display_control |= DISPLAY_ON;
    Send_Command(DISPLAY_CONTROL | display_control);
}

void xLCD_I2C::DisplayOff() noexcept
{
    display_control &= ~DISPLAY_ON;
    Send_Command(DISPLAY_CONTROL | display_control);
}

void xLCD_I2C::BacklightOn() noexcept
{
    backlight = BACKLIGHT;
    I2C_Write_Byte(backlight);
}

void xLCD_I2C::BacklightOff() noexcept
{
    backlight = NO_BACKLIGHT;
    I2C_Write_Byte(backlight);
}

void xLCD_I2C::SetBacklight(bool light_on) noexcept
{
    if (light_on)
    {
        BacklightOn();
...
void xLCD_I2C::CursorOn() noexcept
{
    display_control |= CURSOR_ON;
    Send_Command(DISPLAY_CONTROL | display_control);
}

void xLCD_I2C::CursorOff() noexcept
{
    display_control &= ~CURSOR_ON;
    Send_Command(DISPLAY_CONTROL | display_control);
}

void xLCD_I2C::CursorBlinkOn() noexcept
{
    display_control |= BLINK_ON;
    Send_Command(DISPLAY_CONTROL | display_control);
}

void xLCD_I2C::CursorBlinkOff() noexcept
{
    display_control &= ~BLINK_ON;
    Send_Command(DISPLAY_CONTROL | display_control);
}

void xLCD_I2C::SetTextLeftToRight() noexcept
{
    display_mode |= ENTRY_LEFT;
    Send_Command(ENTRY_MODE_SET | display_mode);
}

void xLCD_I2C::SetTextRightToLeft() noexcept
{
    display_mode &= ~ENTRY_LEFT;
    Send_Command(ENTRY_MODE_SET | display_mode);
}

void xLCD_I2C::Clear() const noexcept
{
    Send_Command(CLEAR_DISPLAY);
}

void xLCD_I2C::Home() const noexcept
{
    Send_Command(RETURN_HOME);
}

void xLCD_I2C::SetCursor(byte row, byte column) const noexcept
{
...
void xLCD_I2C::PrintChar(byte character) const noexcept
{
    Send_Char(character);
}

void xLCD_I2C::PrintString(std::string_view str) const noexcept
{
...
void xLCD_I2C::PrintCustomChar(byte location) const noexcept
{
    Send_Register_Select(location);
}

void xLCD_I2C::CreateCustomChar(byte location, array const char_map) const noexcept
{
